...
@property (nonatomic, weak) TestUtils *qosMgr;
...
- (IBAction)httpAction:(id)sender {
    if (_httpTesting) {
        _httpTesting = NO;
        [[TestUtils getSharedInstance] stopHttpDownloadFile];
        [self enabledAllButton];
    } else {
        _httpTesting = YES;
        [self buttonTitleChange];
...
//        _chartView.data = nil;
        ...
        WeakSelf;
        [[TestUtils getSharedInstance] httpDownloadFile:netSpeed state:@"1" progress:^(NSProgress *_Nonnull downloadProgress) {
...
- (TestUtils *)qosMgr {
    _qosMgr = [TestUtils sharedInstance:_ipTextField.text port:_portTextField.text appId:testAppId userId:testUserId businessId:testBusinessId businessState:@"1"];
    return _qosMgr;
}
...
- (IBAction)traceAction:(id)sender {
    if (_traceTesting) {
        _traceTesting = NO;
        [[TestUtils getSharedInstance] stopTrace];
        [self enabledAllButton];
    } else if ([self ipCheck]) {
        _traceTesting = YES;
        [self buttonTitleChange];
...
//        _chartView.data = nil;
        ...
        WeakSelf;
        TestUtils *tu = [TestUtils sharedInstance:weakSelf.ipTextField.text port:weakSelf.portTextField.text appId:testAppId userId:testUserId businessId:testBusinessId businessState:@"1"];
        [tu trace:^(TracerouteRecord *record) {
            dispatch_async(dispatch_get_main_queue(), ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                               ...
                               [[TestUtils getSharedInstance] uploadTestResult:@"TRACE" port:weakSelf.portTextField.text duration:weakSelf.timeTextField.text testParams:params completionHandler:^(NSURLResponse *_Nonnull response, id _Nullable responseObject, NSError *_Nullable error) {
                               }];
