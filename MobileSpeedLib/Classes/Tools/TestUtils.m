...
@property (copy, nonatomic) NSString *userAreaCode;
...
- (void)udpTest:(NSUInteger)count delegate:(id<MSLGCDAsyncUdpSocketDelegate>)aDelegate state:(NSString *)state automaticStop:(BOOL)automaticStop {
    ...
    __block NSInteger countdown = count;
...
- (void)uploadTestResult:(NSString *_Nonnull)method port:(NSString *)port duration:(NSString *)duration testParams:(NSDictionary *)testPrarms completionHandler:(nullable void (^)(NSURLResponse *response, id _Nullable responseObject, NSError *_Nullable error))completionHandler {
    if (_speedUpUtils) {
        ...
        if (_userAreaCode == nil || _userAreaCode.length == 0) {
            params[@"areacode"] = infoModel.areaInfo.areaId;
        } else {
            params[@"areacode"] = _userAreaCode;
        }
