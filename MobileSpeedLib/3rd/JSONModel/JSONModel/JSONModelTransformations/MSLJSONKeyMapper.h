 ...
 *
 * In your model overwrite the + (MSLJSONKeyMapper *)keyMapper method and provide a MSLJSONKeyMapper
 * instance to convert the key names for your model.
 *
 * If you need custom mapping it's as easy as:
 * <pre>
 * + (MSLJSONKeyMapper *)keyMapper {
 * &nbsp; return [[MSLJSONKeyMapper&nbsp;alloc]&nbsp;initWithDictionary:@{@"crazy_JSON_name":@"myCamelCaseName"}];
 * }
 * </pre>
 * In case you want to handle underscore_case, **use the predefined key mapper**, like so:
 * <pre>
 * + (MSLJSONKeyMapper *)keyMapper {
 * &nbsp; return [MSLJSONKeyMapper&nbsp;mapperFromUnderscoreCaseToCamelCase];
 * }
 * </pre>
 */
@interface MSLJSONKeyMapper : NSObject
...
+ (instancetype)mapper:(MSLJSONKeyMapper *)baseKeyMapper withExceptions:(NSDictionary *)exceptions DEPRECATED_MSG_ATTRIBUTE("use baseMapper:withModelToJSONExceptions:");
 ...
 * @return MSLJSONKeyMapper instance
 */
...
/**
 * Creates a MSLJSONKeyMapper instance, based on the block you provide this initializer.
...
/**
 * Creates a MSLJSONKeyMapper instance, based on the mapping you provide.
...
/**
 * Creates a MSLJSONKeyMapper based on a built-in MSLJSONKeyMapper, with specific exceptions.
 * Use your JSONModel property names as keys, and the JSON key names as values.
 */
+ (instancetype)baseMapper:(MSLJSONKeyMapper *)baseKeyMapper withModelToJSONExceptions:(NSDictionary *)toJSON;
