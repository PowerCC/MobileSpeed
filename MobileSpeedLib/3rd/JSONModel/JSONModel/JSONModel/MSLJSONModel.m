...
#import "MSLJSONModelClassProperty.h"
...
static MSLJSONValueTransformer* valueTransformer = nil;
static Class JSONModelClass = NULL;

#pragma mark - model cache
static MSLJSONKeyMapper* globalKeyMapper = nil;

#pragma mark - JSONModel implementation
@implementation MSLJSONModel
{
...
+(void)load
{
    ...
    dispatch_once(&once, ^{
        ...
        @autoreleasepool {
            ...
            valueTransformer = [[MSLJSONValueTransformer alloc] init];
...
-(instancetype)initWithData:(NSData *)data error:(NSError *__autoreleasing *)err
{
    //check for nil input
    if (!data) {
        if (err) *err = [MSLJSONModelError errorInputIsNil];
        return nil;
    }
    //read the json
    ...
    if (initError) {
        if (err) *err = [MSLJSONModelError errorBadJSON];
        return nil;
    }
...
-(id)initWithString:(NSString*)string error:(MSLJSONModelError**)err
{
    MSLJSONModelError* initError = nil;
...
-(id)initWithString:(NSString *)string usingEncoding:(NSStringEncoding)encoding error:(MSLJSONModelError**)err
{
    //check for nil input
    if (!string) {
        if (err) *err = [MSLJSONModelError errorInputIsNil];
...
-(id)initWithDictionary:(NSDictionary*)dict error:(NSError**)err
{
    ...
    if (![dict isKindOfClass:[NSDictionary class]]) {
        if (err) *err = [MSLJSONModelError errorInvalidDataWithMessage:@"Attempt to initialize JSONModel object using initWithDictionary:error: but the dictionary parameter was not an 'NSDictionary'."];
        return nil;
    }
    ...
    if (!self) {
        ...
        if (err) *err = [MSLJSONModelError errorModelIsInvalid];
        return nil;
    }
...
-(MSLJSONKeyMapper*)__keyMapper
{
    //get the model key mapper
    return objc_getAssociatedObject(self.class, &kMapperObjectKey);
}

-(BOOL)__doesDictionary:(NSDictionary*)dict matchModelWithKeyMapper:(MSLJSONKeyMapper*)keyMapper error:(NSError**)err
{
    ...
    if (keyMapper || globalKeyMapper) {
        ...
        for (MSLJSONModelClassProperty* property in [self __properties__]) {
    ...
    if (![requiredProperties isSubsetOfSet:incomingKeys]) {
        ...
        if (err) *err = [MSLJSONModelError errorInvalidDataWithMissingKeys:requiredProperties];
        return NO;
    }
...
-(NSString*)__mapString:(NSString*)string withKeyMapper:(MSLJSONKeyMapper*)keyMapper
{
    if (keyMapper) {
        //custom mapper
...
-(BOOL)__importDictionary:(NSDictionary*)dict withKeyMapper:(MSLJSONKeyMapper*)keyMapper validation:(BOOL)validation error:(NSError**)err
{
    ...
    for (MSLJSONModelClassProperty* property in [self __properties__]) {
        ...
        if (isNull(jsonValue)) {
            ...
            if (err) {
                ...
                MSLJSONModelError* dataErr = [MSLJSONModelError errorInvalidDataWithMessage:msg];
        ...
        if (property) {
            ...
            if ([self __isJSONModelSubClass:property.type]) {
                ...
                if (
                    ...
                    Class sourceClass = [MSLJSONValueTransformer classByResolvingClusterClasses:[jsonValue class]];
                    ...
                    if (foundCustomTransformer) {
                        ...
                        if (err) {
                            ...
                            MSLJSONModelError* dataErr = [MSLJSONModelError errorInvalidDataWithTypeMismatch:msg];
                            *err = [dataErr errorByPrependingKeyPathComponent:property.name];
                        }
                        return NO;
                    }
...
-(NSMutableSet*)__requiredPropertyNames
{
    ...
    if (!classRequiredPropertyNames) {
        ...
        [[self __properties__] enumerateObjectsUsingBlock:^(MSLJSONModelClassProperty* p, NSUInteger idx, BOOL *stop) {
...
-(void)__inspectProperties
{
    ...
    while (class != [MSLJSONModel class]) {
        ...
        for (unsigned int i = 0; i < propertyCount; i++) {
            ...
            MSLJSONModelClassProperty* p = [[MSLJSONModelClassProperty alloc] init];
...
#pragma GCC diagnostic pop
            ...
            if (p)
            {
                ...
                {
                    NSString *class = NSStringFromClass([MSLJSONValueTransformer classByResolvingClusterClasses:type]);
...
-(id)__transform:(id)value forProperty:(MSLJSONModelClassProperty*)property error:(NSError**)err
{
        ...
        if ([property.type isSubclassOfClass:[NSArray class]]) {
            ...
            if(![[value class] isSubclassOfClass:[NSArray class]])
            {
                if(err != nil)
                {
                    ...
                    MSLJSONModelError* typeErr = [MSLJSONModelError errorInvalidDataWithTypeMismatch:mismatch];
            ...
            //one shot conversion
            MSLJSONModelError* arrayErr = nil;
...
-(id)__reverseTransform:(id)value forProperty:(MSLJSONModelClassProperty*)property
{
    Class protocolClass = NSClassFromString(property.protocol);
...
- (BOOL)__customSetValue:(id <NSObject>)value forProperty:(MSLJSONModelClassProperty *)property
{
    NSString *class = NSStringFromClass([MSLJSONValueTransformer classByResolvingClusterClasses:[value class]]);
...
- (BOOL)__customGetValue:(id *)value forProperty:(MSLJSONModelClassProperty *)property
{
...
- (NSDictionary *)toDictionaryWithKeys:(NSArray <NSString *> *)propertyNames
{
    ...
    for (MSLJSONModelClassProperty* p in properties) {
        ...
        if ([value isKindOfClass:JSONModelClass]) {
            ...
            //recurse models
            value = [(MSLJSONModel*)value toDictionary];
            [tempDictionary setValue:value forKeyPath: keyPath];
            ...
            if (YES) {
                ...
                if (foundCustomTransformer) {
                    ...
                    @throw [NSException exceptionWithName:@"Value transformer not found"
                                                   reason:[NSString stringWithFormat:@"[MSLJSONValueTransformer %@] not found", selectorName]
                                                 userInfo:nil];
                    return nil;
                }
            }
...
+ (NSMutableDictionary *)dictionaryOfModelsFromDictionary:(NSDictionary *)dictionary error:(NSError **)err
{
    ...
    for (NSString *key in dictionary.allKeys)
    {
        ...
        {
            if (err) {
                *err = [MSLJSONModelError errorInvalidDataWithTypeMismatch:@"Only dictionaries and arrays are supported"];
            }
            return nil;
        }
    }
...
-(NSString*)description
{
    ...
    for (MSLJSONModelClassProperty *p in [self __properties__]) {
...
+(MSLJSONKeyMapper*)keyMapper
{
    return nil;
}

+(void)setGlobalKeyMapper:(MSLJSONKeyMapper*)globalKeyMapperParam
{
