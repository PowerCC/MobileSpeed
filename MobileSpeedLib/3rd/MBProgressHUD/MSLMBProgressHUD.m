...
CGFloat const MSLMBProgressMaxOffset = 1000000.f;
...
@implementation MSLMSLMBProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    MSLMSLMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    MSLMSLMBProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (MSLMSLMBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            MSLMSLMBProgressHUD *hud = (MSLMSLMBProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = MSLMBProgressHUDAnimationFade;
    _mode = MSLMBProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(MSLMBProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == MSLMBProgressHUDAnimationZoom) {
        type = animatingIn ? MSLMBProgressHUDAnimationZoomIn : MSLMBProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == MSLMBProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == MSLMBProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == MSLMBProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == MSLMBProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    id<MSLMSLMBProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    MSLMBBackgroundView *backgroundView = [[MSLMBBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = MSLMBProgressHUDBackgroundStyleSolidColor;
    ...
    MSLMBBackgroundView *bezelView = [MSLMBBackgroundView new];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[MSLMBRoundProgressView class]];

    MSLMBProgressHUDMode mode = self.mode;
    if (mode == MSLMBProgressHUDModeIndeterminate) {
    ...
    else if (mode == MSLMBProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[MSLMBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == MSLMBProgressHUDModeDeterminate || mode == MSLMBProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[MSLMBRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == MSLMBProgressHUDModeAnnularDeterminate) {
            [(MSLMBRoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == MSLMBProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == MSLMBProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[MSLMSLMBProgressHUD class]]];
#endif

        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[MSLMBRoundProgressView class]]) {
        MSLMBRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MSLMBRoundProgressView appearanceWhenContainedIn:[MBProgressHUD class], nil];
#else
        appearance = [MSLMBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[MSLMSLMBProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((MSLMBRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((MSLMBRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[MSLMBBarProgressView class]]) {
        MSLMBBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MSLMBBarProgressView appearanceWhenContainedIn:[MBProgressHUD class], nil];
#else
        appearance = [MSLMBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[MSLMSLMBProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((MSLMBBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((MSLMBBarProgressView *)indicator).lineColor = color;
        }
    } else {
...
- (void)updateBezelMotionEffects {
    MSLMBBackgroundView *bezelView = self.bezelView;
...
- (void)setMode:(MSLMBProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == MSLMBProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation MSLMBRoundProgressView
...
@implementation MSLMBBarProgressView
...
@implementation MSLMBBackgroundView

#pragma mark - Lifecycle

- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        _style = MSLMBProgressHUDBackgroundStyleBlur;
...
- (void)setStyle:(MSLMBProgressHUDBackgroundStyle)style {
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
    }
...
- (void)updateForBackgroundStyle {
    ...
    MSLMBProgressHUDBackgroundStyle style = self.style;
    if (style == MSLMBProgressHUDBackgroundStyleBlur) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == MSLMBProgressHUDBackgroundStyleBlur) {
        self.backgroundColor = self.color;
    } else {
        self.backgroundColor = self.color;
    }
