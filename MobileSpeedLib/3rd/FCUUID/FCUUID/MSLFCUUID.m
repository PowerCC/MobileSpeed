...
#import "MSLFCUUID.h"
#import "MSLUICKeyChainStore.h"


@implementation MSLFCUUID


NSString *const MSLFCUUIDsOfUserDevicesDidChangeNotification = @"MSLFCUUIDsOfUserDevicesDidChangeNotification";


NSString *const _mslUuidForInstallationKey = @"fc_uuidForInstallation";
NSString *const _mslUuidForDeviceKey = @"fc_uuidForDevice";
NSString *const _mslUuidsOfUserDevicesKey = @"fc_uuidsOfUserDevices";
NSString *const _mslUuidsOfUserDevicesToggleKey = @"fc_uuidsOfUserDevicesToggle";


+(MSLFCUUID *)sharedInstance
{
    static MSLFCUUID *instance = nil;
...
-(NSString *)_getValueForKey:(NSString *)key userDefaults:(BOOL)userDefaults keychain:(BOOL)keychain service:(NSString *)service accessGroup:(NSString *)accessGroup
{
    ...
    if(!value && keychain ){
        value = [MSLUICKeyChainStore stringForKey:key service:service accessGroup:accessGroup];
    }
...
-(void)_setValue:(NSString *)value forKey:(NSString *)key userDefaults:(BOOL)userDefaults keychain:(BOOL)keychain service:(NSString *)service accessGroup:(NSString *)accessGroup synchronizable:(BOOL)synchronizable
{
    ...
    if( value && keychain ){
        MSLUICKeyChainStore *keychain = [MSLUICKeyChainStore keyChainStoreWithService:service accessGroup:accessGroup];
...
-(NSString *)uuidForInstallation
{
    if( _uuidForInstallation == nil ){
        _uuidForInstallation = [self _getOrCreateValueForKey:_mslUuidForInstallationKey defaultValue:nil userDefaults:YES keychain:NO service:nil accessGroup:nil synchronizable:NO];
    }
...
-(void)uuidForDevice_updateWithValue:(NSString *)value
{
    ...
    [self _setValue:_uuidForDevice forKey:_mslUuidForDeviceKey userDefaults:YES keychain:YES service:nil accessGroup:nil synchronizable:NO];
}
...
-(NSString *)uuidForDevice
{
    ...
    if( _uuidForDevice == nil ){
        _uuidForDevice = [self _getOrCreateValueForKey:_mslUuidForDeviceKey defaultValue:nil userDefaults:YES keychain:YES service:nil accessGroup:nil synchronizable:NO];
    }
...
-(void)uuidsOfUserDevices_iCloudSync
{
    if( _uuidsOfUserDevices_iCloudAvailable )
    {
        ...
        {
            NSString *uuidOfUserDeviceAsKey = [NSString stringWithFormat:@"%@_%@", _mslUuidForDeviceKey, uuidOfUserDevice];
        ...
        [iCloud setBool:![iCloud boolForKey:_mslUuidsOfUserDevicesToggleKey] forKey:_mslUuidsOfUserDevicesToggleKey];
        [iCloud synchronize];
    }
}
...
-(void)uuidsOfUserDevices_iCloudChange:(NSNotification *)notification
{
    if( _uuidsOfUserDevices_iCloudAvailable )
    {
        ...
        [iCloudDict enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
            ...
            if([uuidKey rangeOfString:_mslUuidForDeviceKey].location == 0)
            {
        ...
        if([uuidsSet count] > uuidsCount)
        {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:MSLFCUUIDsOfUserDevicesDidChangeNotification object:self userInfo:userInfo];
        }
    }
}
...
-(void)uuidsOfUserDevices_updateWithValue:(NSArray *)value
{
    ...
    [self _setValue:_uuidsOfUserDevices forKey:_mslUuidsOfUserDevicesKey userDefaults:YES keychain:YES service:nil accessGroup:nil synchronizable:YES];
}
...
-(NSArray *)uuidsOfUserDevices
{
    if( _uuidsOfUserDevices == nil ){
        _uuidsOfUserDevices = [self _getOrCreateValueForKey:_mslUuidsOfUserDevicesKey defaultValue:[self uuidForDevice] userDefaults:YES keychain:YES service:nil accessGroup:nil synchronizable:YES];
    }
