...
@implementation MSLPNTcpPingResult
...
static MSLPNTcpPing *g_tcpPing = nil;
...
@property (copy,readonly) MSLPNTcpPingHandler complete;
...
@implementation MSLPNTcpPing

- (instancetype)init:(NSString *)host
                port:(NSUInteger)port
               count:(NSUInteger)count
            complete:(MSLPNTcpPingHandler)complete
{
    if (self = [super init]) {
        _host = host;
        _port = port;
...
+ (instancetype)start:(NSString * _Nonnull)host
             complete:(MSLPNTcpPingHandler _Nonnull)complete
{
...
+ (instancetype)start:(NSString * _Nonnull)host
             ...
             complete:(MSLPNTcpPingHandler _Nonnull)complete
{
    MSLPNTcpPing *tcpPing = [[MSLPNTcpPing alloc] init:host port:port count:count complete:complete];
    g_tcpPing = tcpPing;
...
//            NSLog(@"connected to %s:%lu, %f ms\n",inet_ntoa(addr.sin_addr), (unsigned long)_port, conn_time * 1000);
    ...
    dispatch_async(dispatch_get_main_queue(), ^(void) {
        ...
        if (self.isSucc) {
            MSLPNTcpPingResult *pingRes  = [self constPingRes:code ip:ip durations:intervals loss:loss count:index];
...
- (MSLPNTcpPingResult *)constPingRes:(NSInteger)code
                               ip:(NSString *)ip
...
{
    if (code != 0 && code != -5) {
        return [[MSLPNTcpPingResult alloc] init:ip loss:1 count:1 max:0 min:0 avg:0];
    }
    ...
    return [[MSLPNTcpPingResult alloc] init:ip loss:loss count:count max:max min:min avg:avg];
}
@end
