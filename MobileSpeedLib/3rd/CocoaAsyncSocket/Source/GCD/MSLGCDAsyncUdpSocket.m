...
@class MSLGCDAsyncUdpSendPacket;
...
@interface MSLGCDAsyncUdpSocket ()
{
	...
	MSLGCDAsyncUdpSendPacket *currentSend;
...
**/
@interface MSLGCDAsyncUdpSendPacket : NSObject {
@public
	NSData *buffer;
...
@implementation MSLGCDAsyncUdpSendPacket
...
@interface MSLGCDAsyncUdpSpecialPacket : NSObject {
@public
//	uint8_t type;
...
@implementation MSLGCDAsyncUdpSpecialPacket
...
@implementation MSLGCDAsyncUdpSocket
...
- (instancetype)initWithDelegate:(id<MSLGCDAsyncUdpSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq
{
	LogTrace();
...
- (instancetype)initWithDelegate:(id<MSLGCDAsyncUdpSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	LogTrace();
...
- (id<MSLGCDAsyncUdpSocketDelegate>)delegate
{
	if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
	{
		return delegate;
...
- (void)setDelegate:(id<MSLGCDAsyncUdpSocketDelegate>)newDelegate synchronously:(BOOL)synchronously
{
...
- (void)setDelegate:(id<MSLGCDAsyncUdpSocketDelegate>)newDelegate
{
	[self setDelegate:newDelegate synchronously:NO];
}

- (void)synchronouslySetDelegate:(id<MSLGCDAsyncUdpSocketDelegate>)newDelegate
{
...
- (void)getDelegate:(id<MSLGCDAsyncUdpSocketDelegate> *)delegatePtr delegateQueue:(dispatch_queue_t *)delegateQueuePtr
{
	if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
	{
...
- (void)setDelegate:(id<MSLGCDAsyncUdpSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue synchronously:(BOOL)synchronously
{
	dispatch_block_t block = ^{
...
- (void)setDelegate:(id<MSLGCDAsyncUdpSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue
{
	[self setDelegate:newDelegate delegateQueue:newDelegateQueue synchronously:NO];
}

- (void)synchronouslySetDelegate:(id<MSLGCDAsyncUdpSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue
{
...
- (void)notifyDidConnectToAddress:(NSData *)anAddress
{
	...
	__strong id<MSLGCDAsyncUdpSocketDelegate> theDelegate = delegate;
...
- (BOOL)connectToHost:(NSString *)host onPort:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		MSLGCDAsyncUdpSpecialPacket *packet = [[MSLGCDAsyncUdpSpecialPacket alloc] init];
...
- (void)maybeConnect
{
	...
	BOOL sendQueueReady = [currentSend isKindOfClass:[MSLGCDAsyncUdpSpecialPacket class]];
	
	if (sendQueueReady)
	{
		MSLGCDAsyncUdpSpecialPacket *connectPacket = (MSLGCDAsyncUdpSpecialPacket *)currentSend;
...
- (void)sendData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	MSLGCDAsyncUdpSendPacket *packet = [[MSLGCDAsyncUdpSendPacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)sendData:(NSData *)data toAddress:(NSData *)remoteAddr withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	MSLGCDAsyncUdpSendPacket *packet = [[MSLGCDAsyncUdpSendPacket alloc] initWithData:data timeout:timeout tag:tag];
	packet->addressFamily = [MSLGCDAsyncUdpSocket familyFromAddress:remoteAddr];
...
- (void)maybeDequeueSend
{
	...
	if (currentSend == nil)
	{
		...
		while ([sendQueue count] > 0)
		{
			...
			if ([currentSend isKindOfClass:[MSLGCDAsyncUdpSpecialPacket class]])
			{
				[self maybeConnect];
...
- (void)doPreSend
{
	...
	if (sendFilterBlock && sendFilterQueue)
	{
		...
		if (sendFilterAsync)
		{
			...
			MSLGCDAsyncUdpSendPacket *sendPacket = currentSend;
...
+ (void)addStreamListener:(MSLGCDAsyncUdpSocket *)asyncUdpSocket
{
	LogTrace();
...
+ (void)removeStreamListener:(MSLGCDAsyncUdpSocket *)asyncUdpSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == listenerThread, @"Invoked on wrong thread");
...
{
	@autoreleasepool {
		MSLGCDAsyncUdpSocket *asyncUdpSocket = (__bridge MSLGCDAsyncUdpSocket *)pInfo;
