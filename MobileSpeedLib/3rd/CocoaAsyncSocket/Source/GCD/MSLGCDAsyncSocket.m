...
@interface MSLGCDAsyncSocketPreBuffer : NSObject
{
	uint8_t *preBuffer;
...
@implementation MSLGCDAsyncSocketPreBuffer
...
**/
@interface MSLGCDAsyncReadPacket : NSObject
{
  @public
...
- (NSUInteger)readLengthForTermWithPreBuffer:(MSLGCDAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr;

- (NSInteger)searchForTermAfterPreBuffering:(ssize_t)numBytes;

@end

@implementation MSLGCDAsyncReadPacket
...
**/
@interface MSLGCDAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation MSLGCDAsyncWritePacket
...
**/
@interface MSLGCDAsyncSpecialPacket : NSObject
{
  @public
...
@implementation MSLGCDAsyncSpecialPacket
...
@implementation MSLGCDAsyncSocket
{
	uint32_t flags;
	uint16_t config;
	...
	MSLGCDAsyncReadPacket *currentRead;
	MSLGCDAsyncWritePacket *currentWrite;
	
	unsigned long socketFDBytesAvailable;
	
	MSLGCDAsyncSocketPreBuffer *preBuffer;
	...
	MSLGCDAsyncSocketPreBuffer *sslPreBuffer;
...
- (instancetype)initWithDelegate:(id<GCDAsyncSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		...
		preBuffer = [[MSLGCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<GCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError* __autoreleasing *)error
{
  ...
  MSLGCDAsyncSocket *socket = [[[self class] alloc] initWithDelegate:aDelegate delegateQueue:dq socketQueue:sq];
...
- (BOOL)acceptOnInterface:(NSString *)inInterface port:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (enableIPv4)
		{
			...
			__weak MSLGCDAsyncSocket *weakSelf = self;
			
            dispatch_source_set_event_handler(self->accept4Source, ^{ @autoreleasepool {
			#pragma clang diagnostic push
			#pragma clang diagnostic warning "-Wimplicit-retain-self"
				
				__strong MSLGCDAsyncSocket *strongSelf = weakSelf;
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			MSLGCDAsyncSocket *acceptedSocket = [[[self class] alloc] initWithDelegate:theDelegate
                        ...
                        tag:(long)tag
{
	...
	MSLGCDAsyncReadPacket *packet = [[MSLGCDAsyncReadPacket alloc] initWithData:buffer
	                                                          startOffset:offset
	                                                            maxLength:length
	                                                              timeout:timeout
	                                                           readLength:0
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
        ...
        if (!self->currentRead || ![self->currentRead isKindOfClass:[MSLGCDAsyncReadPacket class]])
		{
...
- (void)maybeDequeueRead
{
	...
	if ((currentRead == nil) && (flags & kConnected))
	{
		if ([readQueue count] > 0)
		{
			...
			if ([currentRead isKindOfClass:[MSLGCDAsyncSpecialPacket class]])
			{
...
- (void)completeCurrentRead
{
	...
	{
		MSLGCDAsyncReadPacket *theRead = currentRead; // Ensure currentRead retained since result may not own buffer
...
- (void)doReadTimeout
{
	...
	{
		MSLGCDAsyncReadPacket *theRead = currentRead;
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	MSLGCDAsyncWritePacket *packet = [[MSLGCDAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (float)progressOfWriteReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
        ...
        if (!self->currentWrite || ![self->currentWrite isKindOfClass:[MSLGCDAsyncWritePacket class]])
		{
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		if ([writeQueue count] > 0)
		{
			...
			if ([currentWrite isKindOfClass:[MSLGCDAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued GCDAsyncSpecialPacket");
...
- (void)doWriteTimeout
{
	...
	{
		MSLGCDAsyncWritePacket *theWrite = currentWrite;
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	MSLGCDAsyncSpecialPacket *packet = [[MSLGCDAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		#if TARGET_OS_IPHONE
		{
			MSLGCDAsyncSpecialPacket *tlsPacket = (MSLGCDAsyncSpecialPacket *)currentRead;
            NSDictionary *tlsSettings = @{};
            if (tlsPacket) {
...
{
	MSLGCDAsyncSocket *asyncSocket = (__bridge MSLGCDAsyncSocket *)connection;
...
- (void)ssl_startTLS
{
	...
	sslPreBuffer = [[MSLGCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
- (void)cf_startTLS
{
	...
	NSAssert([currentRead isKindOfClass:[MSLGCDAsyncSpecialPacket class]], @"Invalid read packet for startTLS");
	NSAssert([currentWrite isKindOfClass:[MSLGCDAsyncSpecialPacket class]], @"Invalid write packet for startTLS");
...
+ (void)scheduleCFStreams:(MSLGCDAsyncSocket *)asyncSocket
{
	LogTrace();
...
+ (void)unscheduleCFStreams:(MSLGCDAsyncSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
...
{
	MSLGCDAsyncSocket *asyncSocket = (__bridge MSLGCDAsyncSocket *)pInfo;
