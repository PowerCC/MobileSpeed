...
@class MSLGCDAsyncReadPacket;
@class MSLGCDAsyncWritePacket;
@class MSLGCDAsyncSocketPreBuffer;
...
@interface MSLGCDAsyncSocket : NSObject
...
**/
- (nullable dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(MSLGCDAsyncSocket *)sock;
...
**/
- (void)socket:(MSLGCDAsyncSocket *)sock didAcceptNewSocket:(MSLGCDAsyncSocket *)newSocket;

/**
 * Called when a socket connects and is ready for reading and writing.
 * The host parameter will be an IP address, not a DNS name.
**/
- (void)socket:(MSLGCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port;

/**
 * Called when a socket connects and is ready for reading and writing.
 * The host parameter will be an IP address, not a DNS name.
 **/
- (void)socket:(MSLGCDAsyncSocket *)sock didConnectToUrl:(NSURL *)url;

/**
 * Called when a socket has completed reading the requested data into memory.
 * Not called if there is an error.
**/
- (void)socket:(MSLGCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)socket:(MSLGCDAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(MSLGCDAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used for things such as updating progress bars.
**/
- (void)socket:(MSLGCDAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(MSLGCDAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(MSLGCDAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(MSLGCDAsyncSocket *)sock;
...
**/
- (void)socketDidDisconnect:(MSLGCDAsyncSocket *)sock withError:(nullable NSError *)err;
...
**/
- (void)socketDidSecure:(MSLGCDAsyncSocket *)sock;
...
**/
- (void)socket:(MSLGCDAsyncSocket *)sock didReceiveTrust:(SecTrustRef)trust
